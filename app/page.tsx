...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-800 font-sans flex flex-col">
      ...
      {/* Ayrılmış Araçların Kart Listesi (Convertworld Tarzı) */}
        ...
        {Object.keys(categories).map((key) => {
          ...
          return (
            <a 
              key={key} 
              href={`/convert/${key}`}
              className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm hover:shadow-md hover:border-blue-400 transition flex flex-col justify-between group"
            >
              <div>
                <div className="text-4xl mb-4 group-hover:scale-110 transition-transform w-fit">{cat.name[lang].split(' ')[1] || '⚙️'}</div>
                <h2 className="text-xl font-bold text-gray-900 mb-2">{cat.name[lang].split(' ')[0]}</h2>
                <p className="text-xs text-gray-400 uppercase tracking-wider">
                  {Object.keys(cat.units).length} Units Available
                </p>
              </div>
      ...
      <footer className="border-t border-gray-200 bg-white py-6 text-center text-xs text-gray-400 mt-12">
        &copy; 2026 OptiConv. International Standard Metrics. All rights reserved.
      </footer>
    </div>
  );
}
